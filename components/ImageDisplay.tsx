...
export const ImageDisplay: React.FC<ImageDisplayProps> = ({ imageSrc, onSelect, isEnhancing, historicalSelection, useFixedSelectionBox, fixedSelectionSizePercentage }) => {
  ...
  const draw = useCallback(() => {
    ...
    if (selection) {
      ctx.strokeStyle = '#1a73e8'; // Studio blue
      ctx.lineWidth = 2;
      ctx.setLineDash([4, 4]);
      ...
      const info = `${Math.round(selection.w)} × ${Math.round(selection.h)}`;
      const textMetrics = ctx.measureText(info);
      ctx.fillStyle = '#1a73e8';
      ctx.fillRect(selection.x - 1, selection.y - 18, textMetrics.width + 8, 18);
      ctx.fillStyle = '#FFFFFF';
      ctx.fillText(info, selection.x + 3, selection.y - 5);
      ...
      ctx.fillRect(screenRect.x - 1, screenRect.y - 16, textMetrics.width + 6, 16);
      ctx.fillStyle = 'rgba(255, 255, 255, 0.6)';
      ctx.fillText(info, screenRect.x + 2, screenRect.y - 4);
    }
